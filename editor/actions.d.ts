import { BoxData, BoxSize, CursorPosition, Delta, FontData, LayerComponentProps, LayerId, PageSize, SerializedLayer, SerializedLayerTree, SerializedPage } from '@/app/libs/design-core';
...
export declare const ActionMethods: (state: EditorState, query: CoreEditorQuery) => {
    setProp<T extends LayerComponentProps>(pageIndex: number, layerId: LayerId | LayerId[], props: DeepPartial<T>, customizer?: (objVal: unknown, srcVal: unknown) => unknown): void;
    ...
    moveLayerPosition: (pageIndex: number, layerId: LayerId, newPosition: number) => void;
    ...
    addLineLayer(serializedLayer: Pick<SerializedLayer, 'props'>, parentId?: LayerId): void;
    ...
    setUpdateLineData: (status: boolean, layerId?: LayerId, linePosition?: LinePosition) => void;
    setControlBox: (data?: BoxData) => void;
    setSidebar: (sidebar?: SidebarType) => void;
    openImageEditor(pageIndex: number, layerId: LayerId, { boxSize, position, rotate, image, video, }: {
        position: Delta;
        rotate: number;
        boxSize: BoxSize;
        image?: {
